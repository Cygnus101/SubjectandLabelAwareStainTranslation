...
from src.models.Backbone_model.CycleGANv2 import UNetGenerator, Discriminator
...
def gradient_penalty(discriminator, real, fake, device):
    """Compute WGAN-GP penalty for a discriminator."""
    real = real.detach()
    fake = fake.detach()
    ...
    eps = torch.rand(batch_size, 1, 1, 1, device=device, dtype=real.dtype)
        ...
        for _, (real_H, real_R, _) in enumerate(loop):
            ...
            if not warmup_phase:
                for _ in range(args.d_steps):
                    with torch.no_grad():
                        fake_R_detached = G_H2R(real_H)
                        fake_H_detached = G_R2H(real_R)
                    fake_R_buffer = buffer_fake_R.push_and_pop(fake_R_detached.detach())
                    fake_H_buffer = buffer_fake_H.push_and_pop(fake_H_detached.detach())
                    ...
                    with autocast_ctx():
                        ...
                        loss_D = (D_H_fake.mean() - D_H_real.mean()) + (D_R_fake.mean() - D_R_real.mean())
                        if args.lambda_gp > 0:
                            gp_H = gradient_penalty(D_H, real_H_in, fake_H_in, device)
                            gp_R = gradient_penalty(D_R, real_R_in, fake_R_in, device)
                            loss_D = loss_D + args.lambda_gp * 0.5 * (gp_H + gp_R)
            ...
            for _ in range(args.g_steps):
                with autocast_ctx():
                    ...
                    if warmup_phase:
                        ...
                        adv_loss_val = -D_H(fake_H).mean() - D_R(fake_R).mean()
                        loss_G = adv_loss_val + args.lambda_cycle * loss_cycle + args.lambda_identity * loss_id
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--lambda-gp", type=float, default=10.0, help="Gradient penalty coefficient for WGAN-GP.")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    if args.lambda_gp < 0:
        raise ValueError("--lambda-gp must be non-negative.")
    return args
